...
const CallDialog = ({
  ...
  const [call_details] = useSelector(
    (state: RootState) => state.audioCall.call_queue
  );
  const { incoming } = useSelector((state: RootState) => state.audioCall);
  ...
  const handleDisconnect = (event?: any, reason?: string) => {
    if (reason && reason === "backdropClick") {
      return;
    } else {
      dispatch(ResetAudioCallQueue() as unknown as AnyAction);
